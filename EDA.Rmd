...
```
```{r}
#Plot the outliers in numercial variables
train_num<-select_if(train,is.numeric)%>%str(Train)dplyr::select(-id)
train_num_p<-train_num %>% gather(variable,values,1:18 )
...
#We can see that the variables departure.delay.in.minutes, arrival-delay-in-minutes , flight distance, and check-in service have outliers. We will decide if to change them later based on model performance. 
```
